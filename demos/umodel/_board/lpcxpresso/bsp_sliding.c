...
bsp_pub_sw_evt(ruint s, ruint debsw)
{
    switch (s)
    {
        ...
        case SENSOR_SW:
            if (debsw == SW_PRESS)
            {
                RKH_SMA_POST_FIFO(sliding, &e_strig, &sensor);
            }
            else
            {
                RKH_SMA_POST_FIFO(sliding, &e_srel, &sensor);
            }
            break;
...
{
    SetPIN(LED1, open ? ON : OFF);
    SetPIN(LED2, close ? ON : OFF);
}
